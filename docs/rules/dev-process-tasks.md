...
completing a PRD or a Technical Discovery.

## Guidelines

## Task Implementation

- **One sub-task (e.g. 2.1 -- you can process sub-subtasks (e.g. 2.1.1) back to
  back until 2.1 is complete) at a time:** Do **NOT** start the next sub‑task
...
- **Completion protocol:**
  1. When you finish a **sub‑task**, immediately mark it as completed by
     changing `[ ]` to `[x]`.
  2. If **all** subtasks underneath a parent task are now `[x]`, also mark the
