...
3. **Assess Current State:** Starting with the files under
   `~/.kilocode/rules/memory-bank`, review the existing codebase to understand
...
4. **Phase 1: Generate Parent Tasks:** Based on the PRD analysis and current
   ...
   tasks to use. It's likely to be between 5 and 7. If we need more than 7, we
   might need to consider breaking PRD into two task files. Present these tasks
   to the user in the specified format (without sub-tasks yet). Make sure to add
   `description`, `priority`, `dependencies`, and `details` for each parent
   tasks. Inform the user: "I have generated the high-level tasks based on the
...
`description`: Overall Task Description `priority`: Perceived Priority
`dependencies`: Task Dependency `details`: More details `testStrategy`: Test
Strategy
