...
| 0.1.3 | Routes resolve      | Open `/`, `/restaurant/r1`, `/owner/r1` in browser | Each route shows the correct page (no 404)              |
...
| #     | Test              | Steps                                          | Expected                                                                                                                                                           |
| ----- | ----------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 0.3.1 | Restaurants API   | `curl http://localhost:8000/api/restaurants`   | 200; JSON with `data` array of 5 restaurants (ids r1–r5), each with `id`, `name`, `address`, `lat`, `lng`, `rating`, `cuisine_type`                                |
...
| 1A.1.2 | Get single restaurant | `curl http://localhost:8000/api/restaurants/r1`          | 200; single restaurant object; response includes `images` array (may be empty) |
...
| #      | Test               | Steps                                                                                                                                           | Expected                                                                                                                                                              |
| ------ | ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1A.2.1 | Import by place_id | `curl -X POST http://localhost:8000/api/restaurants/import -H "Content-Type: application/json" -d '{"place_id":"ChIJAQBEylJYwokRlNgMJJHxNiA"}'` | 200; response has restaurant object with `id`, `name`, `address`, etc.; `restaurant_images.json` has new entries with `source: "google"`, `slot_type: "personalized"` |
...
| #      | Test         | Steps                                                                                                                                                                         | Expected                                                                                                          |
| ------ | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| 1A.3.1 | Add image    | `curl -X POST http://localhost:8000/api/restaurants/r1/images -H "Content-Type: application/json" -d '{"image_url":"https://example.com/photo.jpg","source":"owner_upload"}'` | 201; created image object with `id`, `restaurant_id`, `image_url`, `source`, `tags`, `slot_type`, `display_order` |
| 1A.3.2 | Update image | `curl -X PUT http://localhost:8000/api/restaurants/r1/images/<image_id> -H "Content-Type: application/json" -d '{"slot_type":"intro","tags":["cozy"]}'`                       | 200; updated image returned                                                                                       |
| 1A.3.3 | Delete image | `curl -X DELETE http://localhost:8000/api/restaurants/r1/images/<image_id>`                                                                                                   | 204; image removed from list on next GET                                                                          |
...
| 1A.4.2 | Tag all images for restaurant | `curl -X POST http://localhost:8000/api/restaurants/r1/images/tag-all`                                                       | 200; all images for r1 get tags updated (may take several seconds)                              |

### 2.5 Story template

| #      | Test                   | Steps                                                                                                                                                                                                                      | Expected                                                                     |
| ------ | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| 1A.5.1 | Get template (none)    | `curl http://localhost:8000/api/restaurants/r1/story-template`                                                                                                                                                             | 404 if no template                                                           |
| 1A.5.2 | Create/update template | `curl -X PUT http://localhost:8000/api/restaurants/r1/story-template -H "Content-Type: application/json" -d '{"intro_image_id":"<id>","outro_image_id":"<id>","cta_text":"Book a Table","cta_url":"https://example.com"}'` | 200; saved template returned                                                 |
| 1A.5.3 | Get template (exists)  | `curl http://localhost:8000/api/restaurants/r1/story-template`                                                                                                                                                             | 200; template with `intro_image_id`, `outro_image_id`, `cta_text`, `cta_url` |
...
Use a real restaurant id (e.g. `r1`) that has at least one image for full flow.

### 3.1 Page load & data

| #      | Test             | Steps                   | Expected                                                                                                         |
| ------ | ---------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------- |
| 1A.7.1 | Owner page loads | Navigate to `/owner/r1` | Page loads; restaurant header shows name, address, rating, cuisine types                                         |
| 1A.7.2 | Images displayed | Same page               | Image grid shows all images for r1; each card has thumbnail, tags (pills), slot badge (intro/personalized/outro) |

### 3.2 Image management

| #      | Test              | Steps                                              | Expected                                                                |
| ------ | ----------------- | -------------------------------------------------- | ----------------------------------------------------------------------- |
| 1A.7.3 | Set slot on image | Change slot dropdown on an image to Intro or Outro | Card updates; GET `/api/restaurants/r1` shows updated `slot_type`       |
| 1A.7.4 | Add image         | Use "Add Image" with a valid image URL; submit     | New card appears in grid; POST to `/api/restaurants/r1/images` occurred |
...
| 1A.7.10 | Set CTA and save | Enter CTA text (and optional URL); click "Save Template" | Success; GET `/api/restaurants/r1/story-template` returns saved template       |
| 1A.7.11 | Reload template  | Reload `/owner/r1`                                       | Intro/outro and CTA fields pre-filled from saved template                      |
...
| #      | Test            | Steps                                 | Expected                                                                                                 |
| ------ | --------------- | ------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| 1B.2.1 | Bar visible     | On any route (/, /restaurant/r1)      | Floating persona bar visible (e.g. bottom or top-right) with 3 options (e.g. Vegan, Carnivore, Cocktail) |
...
| #      | Test         | Steps                                                                 | Expected                                                                   |
| ------ | ------------ | --------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| 1B.6.1 | Loading      | Navigate to `/restaurant/r1` (throttle network if needed)             | Skeleton visible until data + preload done; then player appears            |
...
5. **Owner** — Open `/owner/r1`; confirm header and image grid; set intro/outro and CTA; save template.
6. **Re-check story** — Go back to `/restaurant/r1`; story reflects new template (intro/outro/CTA).
...
- [ ] Owner dashboard: load `/owner/r1`; images and template section load; save template works.
